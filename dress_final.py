...
def getQuality(currSubSpace, checkCluster, heomInst, mlc, nlc,  ML, NL):
    ...
    for i in nlc.keys(): 
        for j in nlc[i]:
            ...
            nlDistSum = nlDistSum + heomInst.heom(currSubSpace.values[i], currSubSpace.values[j]); ### ********* possible ERROR ********* (divide before sum?? )
            ## Check if the instances belong to the same cluster or not.
            if checkCluster[i] != checkCluster[j] :
                nlSat += 1;
                
    #print("ML, NL, mlSat, nlSat, nlDistSum, mlDistSum", ML, NL, mlSat, nlSat, nlDistSum, mlDistSum);
    if (ML+NL) > 0:
...
def coreFunction(df, subSpace, subSpaceCatList, algo2, lock, sDistCan, categoryList, mlc, nlc,  ML, NL):
	column = df.columns[0]
	filterCheckPassed = 1;
	Clusters=list();
	subSpaceSet = {};
	...
	if algo2 == 1:
		...
		if  round(sDistStar,5) > round(sDistCan,5):
			sDist = sDistStar
		else:
			sDist = sDistCan;
		...
		if round(newSubSpaceDist,5) <= round(sDist,5):
			filterCheckPassed = 0;
			print("Filter not passed for the subSpace:", keyList);
			
	#print("after filter filterCheckPassed is : ", filterCheckPassed);
	if filterCheckPassed == 1 :
		#print(" In filter check")
		#HDBSCAN
		...
		#clusterer1 = hdbscan.HDBSCAN(min_cluster_size=4, metric=heom_metric1.heom,)
		tempSubSpace = subSpace;
		tempSubSpace.fillna(tempSubSpace.mean(), inplace=True)   ## *************Possible ERROR **********
		...
		#print("Calling quality")
		quality = getQuality(subSpace, subSpaceCluster, heom_metric1, mlc, nlc,  ML, NL)
		print ("************** Quality Score for the subSpace '", keyList, "' is: ", quality );
...
def getSubSpaceClusters(df, subSpace, subSpaceCatList, categoryList, mlc, nlc,  ML, NL):
    ...
    m = mp.Manager();
    l = m.Lock();
    coreFunctionPartial = partial(coreFunction, subSpace=subSpace, subSpaceCatList=subSpaceCatList, algo2=algo2, lock = l, sDistCan = sDistCan ,categoryList = categoryList, mlc = mlc, nlc=nlc,  ML=ML, NL=NL)
    ...
    for i in result:
        for key in i.keys():
            ...
            if round(i[key]["quality"],5) > round(subSpaceSet["qBest"],5):
...
def dressPlus(mlc, nlc, ML, NL):
	...
	while condition is True:
		...
		#print("Category list of keys", categoryList.keys());
		index = 0;
		...
		print("canSubSpaceObjList ", canSubSpaceObjList)
		...
		if ((round(subSpaceCluster["qBest"],5) <= round(qBest,5)) or (subSpaceCluster["qBest"] <= 0.0)) : ## possible its not there.
			condition = False; ## Break the condition because the candidate set is empty.
		elif (round(subSpaceCluster["qBest"],5) > round(qBest),5):
			qBest = subSpaceCluster["qBest"] ;
	f = open("output.txt","a+")        
...
def main():
    print("Parallelization is done with", mp.cpu_count(), "Number of processors")
    mlc = {};
    nlc = {}
    (mlc, nlc,  ML, NL) = readConstraints();
